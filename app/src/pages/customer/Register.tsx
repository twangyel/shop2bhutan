        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                value={form.dzongkhag} disabled={loadingDzongkhags}
                onChange={(e) => update('dzongkhag', e.target.value)}
                className={`w-full h-12 pl-10 pr-4 border rounded-2xl text-sm outline-none transition appearance-none bg-white disabled:bg-gray-50 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 ${errors.dzongkhag ? 'border-red-400' : 'border-gray-300'}`}
              >
                <option value="">{loadingDzongkhags ? 'Loading dzongkhags...' : 'Select dzongkhag'}</option>
                {dzongkhagOptions.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
              </select>
            </div>
